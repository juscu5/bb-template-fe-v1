...
import { Orientation } from "@/models/sales";
...
export const useGeneratePDF = (
  ...
  return () => {
    ...
    const doc = new jsPDF(isLandscape ? "l" : "p", "mm", [pageWidth, pageHeight]);
    ...
    let startY = 10; // Initial Y position accounting for header and table header
    ...
    const addHeader = () => {
      ...
      doc.setFontSize(10);
      doc.setFont("helvetica", "bold");
      const title = dateFrom && dateTo ? "Sales Register By Date" : "Sales Register By Doc Number";
      ...
      const dateRange = dateFrom && dateTo
        ? `Date From: ${dateFrom.toLocaleDateString()} to ${dateTo.toLocaleDateString()}`
        : `Doc No. From: ${docNoFrom} to ${docNoTo}`;
      doc.text(dateRange, startX, startY); // Print date range or document number range
      startY += 5;

      doc.setFontSize(10);
      doc.text(`Date Printed: ${formattedDate}, ${formattedTime}`, startX, startY); // Print date and time of print
    ...
    console.log("adjustedCellWidths", adjustedCellWidths)
    ...
    data.forEach((row) => {
      ...
      tableHead.forEach((header, cellIndex) => {
        const cellValue = row[header.id as keyof SalesItem] || ""; // Get the cell value
        let text = String(cellValue); // Convert value to string
        let textX = headerX;

        if (header.id === "advdoc") {
          // Center-align 'advdoc' data
        ...
        } else if (header.id === "totamtdisfor" && typeof cellValue === "string") {
          // Align 'totamtdisfor' (amount) to the right
    ...
    const totalAmountText = `Total Amount: ${totalAmount.toLocaleString("en-US", {
      minimumFractionDigits: 2,
      maximumFractionDigits: 2,
    })}`;
    ...
    doc.save(
      dateFrom && dateTo
        ? "sales_register_by_date.pdf"
        : "sales_register_by_doc_num.pdf"
    );
  };
};