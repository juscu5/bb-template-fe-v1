...
import { LSTVDialogDeleteForm } from "@/components/lstv-dialog/LSTVDialog";
...
import { LSTVSales } from "./LSTVSales";
...
const systemParam = () => {
  ...
  const enableDocnum = sysParam?.chksaldocnum === 0 ? false : true;
...
const API_CON = () => {
  const { setShowDialog, setShowDeleteDialog } = useHandles();
  ...
  const addData = async (
    ...
    const data = {
      ...
      salesItem: salesItem
        ?.filter((item) => item.itmcde !== "" || item.itmdsc !== "")
        .map((item) => ({
          ...item,
          docnum: formJson["docnum"],
        })),
    };
    ...
    try {
      const res = await ApiService.post(`sales/${docnum}`, data, {
        headers: { Authorization: `Bearer ${account}` },
      });
      if (res.status === 200) {
        setShowDialog!(false);
  ...
  const editData = async (
    ...
    try {
      const res = await ApiService.put("sales/" + formJson["docnum"], data, {
        headers: { Authorization: `Bearer ${account}` },
      });
      if (res.status === 200) {
        setShowDialog!(false);
  ...
  const deleteData = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      const res = await ApiService.delete(`sales/${formJson["docnum"]}`, {
        headers: { Authorization: `Bearer ${account}` },
      });
      if (res.status === 200) {
...
const actionMenu = (): ActionMenu[] => {
  const { handleShowDialog, handleDeleteShowDialog } = useHandles();
  return [
    {
      label: "View",
      icon: eyeFill,
...
const ACTION_DIALOG = () => {
  const { showDeleteDialog, setShowDeleteDialog } = useHandles();
  const { deleteData } = API_CON();
  const FORM_ELEMENTS = formElements();

  return (
    <LSTVDialogDeleteForm
      open={showDeleteDialog!}
      setOpen={(isOpen: boolean) => setShowDeleteDialog!(isOpen)}
...
export default function SalesPage() {
  ...
  const { data } = API_CON();
  const { setShowDialog, showDialog } = useHandles();
  const { addData, editData } = API_CON();
  const FORM_ELEMENTS = formElements();
